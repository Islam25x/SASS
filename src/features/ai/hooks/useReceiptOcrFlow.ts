...
import { buildTransactionsFromReceipt } from "../../transactions/utils/transaction.parser";
...
export function useReceiptOcrFlow(
  ...
  const processReceipt = useCallback(async () => {
    ...
    try {
      ...
      const issuedAt = data.issued_at ?? new Date().toISOString();
      const ids = data.items.map((_, index) =>
        typeof crypto.randomUUID === "function"
          ? crypto.randomUUID()
          : `${Date.now()}-${index}`,
      );

      const normalizedItems = buildTransactionsFromReceipt(data, { issuedAt, ids });
